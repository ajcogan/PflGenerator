...
class PFLGenerator():
    ...
    DEFAULT_ANALOGUE_PLANT_TYPE = 'DNP3 Analogue Input'
    DEFAULT_DIGITAL_PLANT_TYPE = 'DNP3 Digital 2State Normal'
    DEFAULT_CONTROL_PLANT_TYPE = 'DNP3 Control Output Normal'

    output = []

    def add_analogue_input(self, index, description, size=32, plant_type=DEFAULT_ANALOGUE_PLANT_TYPE):
        ## PFL addition of Analogue Input (Card must be selected)
        self.output.extend([['3201'], [30,0,0,index,0,0,size,0,description, description, plant_type]])
        return [['3201'], [30,0,0,index,0,0,size,0,description, description, plant_type]] # Aurora

    def add_binary_input(self, index, description, size=1, plant_type=DEFAULT_DIGITAL_PLANT_TYPE):
        ## PFL addition of Binary Input (Card must be selected)
        j,k = index_conversion(int(index))
        self.output.extend([['3201'], [1,0,0,j,0,0,size,k,description, description, plant_type]])
        return [['3201'], [1,0,0,j,0,0,size,k,description, description, plant_type]] # Aurora

    def add_binary_output(self, index, description, plant_type=DEFAULT_CONTROL_PLANT_TYPE):
        ## PFL addition of Binary Output (Card must be selected)
        self.output.extend([['3300'], [12,1,0,index,0,0,description,]])
        return [['3300'], [12,1,0,index,0,0,description,]]

    # def addBinaryCounter(self, index, description):
    ...
    def add_comment(self, comment):
        ...
        self.output.extend([['999'],[comment]])
        return [['999'],[comment]]

    def select_rtu_card(self, rtu, card):
        ## Selects and active RTU and Card (to then add points to)
        self.output.extend([['3000'], [rtu], ['3103'], [card]])
        return [['3000'], [rtu], ['3103'], [card]]

    def end_script(self):
        ## Identifier for end of script
        self.output.extend([['0'], ['ENDSEC']])
        return [['0'], ['ENDSEC']]

    def delete_scanlink_rowid(self, rowID, component, attribute,*associated):
        ## Delete a scan link base on row ID (Card must be selected)
        self.output.extend([['3203'], [rowID], ['3501'], [component, attribute]+list(associated)])
        return [['3203'], [rowID], ['3501'], [component, attribute]+list(associated)]

    def delete_scanlink_name(self, name, component, attribute, *associated):
         ## Delete a scan link base on link name (Card must be selected and name defined by user)
        self.output.extend([['3202'], [name], ['3501'], [component, attribute]+list(associated)])
        return [['3202'], [name], ['3501'], [component, attribute]+list(associated)]

    def create_scanlink_rowid(self, rowID, component, attribute, *associated):
        ## Add a scan link base on row ID (Card must be selected)
        self.output.extend([['3203'],[rowID],['3500'],[component,attribute]+list(associated)])
        return [['3203'],[rowID],['3500'],[component,attribute]+list(associated)]

    def create_scanlink_name(self, name, component, attribute, *associated):
        ## Add a scan link base on link name (Card must be selected and name defined by user)
        self.output.extend([['3202'], [name], ['3500'], [component,attribute]+ list(associated)])
        return [['3202'], [name], ['3500'], [component,attribute]+ list(associated)]

    def update_scan_point(self, name, propertyID, component, attribute):
        ## Modify the value of selected property of a scan link
        self.output.extend([['3400'],[propertyID],['3202'],[name],['3402'],[component, attribute]])
    ...
    def update_attribute(self, component, attribute, value):
        ...
        self.output.extend([['1101'],[component],['2'],[attribute],['6'],[value]])
        return [['1101'],[component],['2'],[attribute],['6'],[value]]

    def update_digital_index(self, old_index, new_index):
        old_j,old_k = index_conversion(int(old_index))
        new_j,new_k = index_conversion(int(new_index))

        return [['3204'],[1,0,0,old_j,0,0,size,k,description, description, plant_type],['2'],[attribute],['6'],[value]]


    def output_pfl(self, filename):
        ## General writer to CSV file
...
##
    ...
    pfl.select_rtu_card('MC_RTU','G00000697CARD')

    for index, row in df.iterrows():
        pfl.add_binary_input(row['Index'],row['Tag'],row['Size'],row['Plant Type'])

    pfl.end_script()

    pfl.output_pfl('output.pfl')
